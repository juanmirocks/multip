...
class Eval {
  ...
  private var sortedPredictions: List[Prediction] = Nil

  /**
  * For computing fn
  */
  private var totalPositiveParaphrases: Double = 0.0
  ...
  def printPR(): Unit = {
    ...
    for (prediction <- sortedPredictions.sortBy(-_.score)) {
      ...
      fn = totalPositiveParaphrases - tp
      val p = tp / (tp + fp)
      val r = tp / (tp + fn)
      val f = 2 * p * r / (p + r)
    ...
    println(s"# of _actual_ vs _predicted_ paraphrases: ${totalPositiveParaphrases.toInt} vs ${sortedPredictions.length}")
